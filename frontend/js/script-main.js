...
async function pin() {
    if (selectedFiles.size === 1) {
        ...
        await window.pywebview.api.pin_to_quick(files[index].id)
        refresh();
...
async function openFileQuick() {
    if (quickActive) {
        await navigateToBreadcrumbId(null, quickActive)
    }
...
function renderQuick() {
    ...
    quickAccess.forEach(tuple => {
        ...
        d.onclick = (e) => navigateToBreadcrumbId(e, tuple[1]);
...
async function openCliWindow() {
    await window.pywebview.api.open_cli()
}
...
// }
async function navigateToBreadcrumbId(event, id) {
    if (event) {
    ...
    if (currentView === 'list') {
        ...
        gridContent.innerHTML = files.map((file, index) => `
            <div class="file-item-list ${selectedFiles.has(index) ? 'selected' : ''} ${cutcopy.has(index) ? fs_state : ''} ${file.islock ? 'lock' : ''}" 
                ...
                <div class="file-info" ondblclick="openFileHandler('${file.name}')">
                ...
                <div class="file-date">${file.modified_time}</div>
                <div class="file-typeD">${file.type}</div>
